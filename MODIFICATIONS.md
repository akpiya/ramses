...
| **Sink mass in Poisson**      | `pm/rho_fine.f90`: Cloud particles contribute to `rho` via CIC/TSC (`mp=msink/ncloud_sink_massive` for non-direct-force; `mp=0` for direct-force); unchanged by HOLD integrator                     | ❌         |
...
- Do we need to change how direct force particles interact with the gas acceleration?
- TODO:
